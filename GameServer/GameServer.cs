...
public static class GameServer
{
    ...
    private static async Task Main()
    {
        ...
        // 서버 Task 시작
        _ = Task.Run(() => HandleDequeueNetworkData(_cts.Token));
        _ = Task.Run(() => HandleInputAsync(_cts));
        _ = Task.Run(() => SendDataToClientAsync(_cts.Token));

        // DB Task 시작
        DatabaseHandler.Start(_cts.Token);
    ...
    /// </summary>
    private static async Task HandleDequeueNetworkData(CancellationToken cts)
    {
        ...
        try
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                await ApplyNetworkRequest(networkData, cts);
            }
        }
        catch (OperationCanceledException)
        {
            Log.PrintToServer("DequeueHandler Stopped by Cancellation");
        }
    }
    ...
    /// </summary>
    private static async Task ApplyNetworkRequest(NetworkData? data, CancellationToken cts)
    {
        if (data is null)
        {
            Log.PrintToServer("Network Data is null");
            return;
        }
    ...
    private static async Task HandleClientAsync(TcpClient client)
    {
        ...
        try
        {
            while (!_cts.Token.IsCancellationRequested)
            {
                ...
                int lengthRead = await stream.ReadAsync(lengthBuffer, 0, lengthBuffer.Length, _cts.Token);
                if (lengthRead == 0)
                {
        ...
        {
            Log.PrintToServer("HandleClientAsync Stopped by Cancellation for " + GetClientIp(client));
        }
        catch (Exception e)
        {
            Log.PrintToServer("Exception: " + e.Message);
    ...
    /// 클라이언트로 데이터를 전송하는 함수
    /// </summary>
    private static async Task SendDataToClientAsync(CancellationToken cts)
    {
        ...
        try
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    await stream.WriteAsync(lengthBuffer, 0, lengthBuffer.Length, cts);
                    await stream.WriteAsync(buffer, 0, buffer.Length, cts);
        ...
        {
            Log.PrintToServer("SendDataToClientAsync Stopped by Cancellation");
        }
    }
